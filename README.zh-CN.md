...
项目也为获准的集成提供少量开发者接口，但这不是公开网站的主要用途。开发者可从 `/support` 发起申请，通过邮箱验证和 GitHub Issue 提交信息；站点所有者仍会人工审核并单独签发每一把 Key。
...
| `/support` | 自愿赞赏与 API Key 申请入口 |
| `/support/api-key` | 邮箱验证与 API Key 申请跳转页 |
| `/docs` | 开发者接入指南 |
